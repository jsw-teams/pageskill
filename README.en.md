...
- Cookie choices reuse the provided plugin. Optional categories start disabled, and trusted `gatedScripts` are declared by `plugin.defaults` and its schema in `themes/<name>/plugins/cookies/index.ts`, then configured in `themes/<name>/theme.yml`; site config stores policy/controller data. Withdrawal cannot undo an action a script already performed.
