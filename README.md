...
- [我们如何构建插件：以 Cookie 选择器为例](content/posts/cookies/zh-sg.md)
...
- Cookie 选择器复用现成插件；可选类别默认关闭，选择器会按类别显示提供者和保存期限。代码在 `themes/<name>/plugins/cookies/index.ts` 登记能力、资源和 schema，`themes/<name>/theme.yml` 负责 provider 实例；内置支持 Google Analytics、Google Ads、Cloudflare Web Analytics、reCAPTCHA/hCaptcha/Turnstile 和按需 X 嵌入，同时允许未来 provider 扩展字段。所有可选资源都要在同意后加载，site key/token 等公开标识仍是数据，secret 和服务端验证留在私有 backend。`config.yml` 不进入 `dist/public`，也没有线上写入路由。它借鉴政策生成器的透明字段展示，但访客选择器不冒充法律政策生成器；站点政策仍由 `content/pages/privacy/` 维护，访客撤回同意不能撤销已经执行的脚本动作。完整构建过程见 Cookie 教程。
