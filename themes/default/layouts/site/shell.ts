...
export function renderShell(context: ThemeShellContext): string {
  ...
  const footerTools = `<nav class="footer-tools" aria-label="${context.escapeHtml(siteMapLabel)}">${renderChromeLinks(context, chrome.footer.before, 'footer-tool-link')}<a class="footer-tool-link" href="/sitemap.xml">${footerIcon(Map)}<span>${context.escapeHtml(siteMapLabel)}</span></a>${privacyPolicy}${privacyTrigger}${renderChromeLinks(context, chrome.footer.after, 'footer-tool-link')}</nav>`;
