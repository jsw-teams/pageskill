import type { ThemeChromeLink, ThemeShellContext } from '../../../../src/theme-api.ts';
...
import { renderPostMeta } from '../../plugins/post-meta/index.ts';
...
export function renderShell(context: ThemeShellContext): string {
  ...
  const footerTools = `<nav class="footer-tools" aria-label="${context.escapeHtml(siteMapLabel)}">${renderChromeLinks(context, chrome.footer.before, 'footer-tool-link')}${context.footerLinks}<a class="footer-tool-link" data-site-map href="/sitemap.xml">${footerIcon(Map)}<span>${context.escapeHtml(siteMapLabel)}</span></a>${privacyPolicy}${privacyTrigger}${renderChromeLinks(context, chrome.footer.after, 'footer-tool-link')}</nav>`;
