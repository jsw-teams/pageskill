...
export const block: ThemeBlockDefinition = {
  ...
  render: (node, context) => {
    ...
    const heading = context.translate(isUpdates ? 'latestUpdates' : 'latestPosts', isUpdates ? 'Latest updates' : 'Latest articles');
    const allPosts = context.translate(isUpdates ? 'allUpdates' : 'allPosts', isUpdates ? 'View all updates' : 'View all articles');
