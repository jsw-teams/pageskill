...
import { BookOpen, ChartLine, Cookie, FileText, Globe, Layers, Map, Palette, Rocket, Settings2, ShieldCheck, type IconNode } from 'lucide';
...
function renderToolComparison(node: DirectiveNode, context: ThemeRenderContext): string {
  ...
  const label = context.translate('toolComparison.label', 'Ways you can get started');
  const note = context.translate('toolComparison.note', 'This table puts common starting points side by side; the Pageskill column comes from this site.');
...
function renderBenchmarkChart(node: DirectiveNode, context: ThemeRenderContext): string {
  ...
  const label = context.translate('benchmarkChart.label', 'Pageskill build time (seconds)');
  const note = context.translate('benchmarkChart.note', 'Values come from the same example content and help show how build size changes.');
...
const blocks: Record<string, ThemeBlockDefinition> = {
  hero: { name: 'hero', schema: { tone: 'string', align: 'string' }, defaults: { tone: 'default', align: 'left' }, render: (node, context) => { validateAttrs(node, blocks.hero); const tone = enumAttr(node, 'tone', ['default', 'brand', 'muted'], 'default'); const align = enumAttr(node, 'align', ['left', 'center', 'right'], 'left'); const image = (name: string) => context.safeUrl(`/assets/${name}`); const artwork = `<div class="hero-art" aria-hidden="true"><span class="hero-art-dot"></span><img class="hero-art-image hero-art-book" src="${image('hero-book.png')}" alt="" width="300" height="300" fetchpriority="high" decoding="async"><img class="hero-art-image hero-art-backpack" src="${image('hero-backpack.png')}" alt="" width="300" height="300" loading="eager" decoding="async"><img class="hero-art-image hero-art-telescope" src="${image('hero-telescope.png')}" alt="" width="300" height="300" loading="eager" decoding="async"></div>`; return `<section class="block hero tone-${context.escapeHtml(tone)} align-${context.escapeHtml(align)}"><div class="hero-copy">${context.renderNodes(node.children)}</div>${artwork}</section>`; } },
  ...
  'compiler-board': { name: 'compiler-board', schema: {}, render: (node, context) => { validateAttrs(node, blocks['compiler-board']); const label = context.translate('compilerBoard', 'How Pageskill works'); return `<section class="block compiler-board" aria-label="${context.escapeHtml(label)}"><div class="compiler-board-grid">${compilerBoardSteps(node.children, context)}</div></section>`; } },
  metrics: { name: 'metrics', schema: {}, render: (node, context) => { validateAttrs(node, blocks.metrics); const label = context.translate('metrics', 'Build stats'); return `<section class="block metrics" aria-label="${context.escapeHtml(label)}"><div class="metrics-grid">${groupedContent(node.children, context).map((content, index) => `<article class="metric-card"><span class="metric-index">${String(index + 1).padStart(2, '0')}</span>${content}</article>`).join('')}</div></section>`; } },
  ...
  'research-matrix': { name: 'research-matrix', schema: {}, render: (node, context) => { validateAttrs(node, blocks['research-matrix']); const label = context.translate('research', 'Feature guide'); return `<section id="research" class="block research-matrix" aria-label="${context.escapeHtml(label)}">${context.renderNodes(node.children)}</section>`; } },
  comparison: { name: 'comparison', schema: {}, render: (node, context) => { validateAttrs(node, blocks.comparison); const cards = groupedContent(node.children, context); if (cards.length < 2) throw new MarkdownError('Block "comparison" needs at least two comparison columns; add two level-three headings with their supporting Markdown', node.position); const label = context.translate('comparison', 'Tool comparison'); return `<section id="comparison" class="block comparison" aria-label="${context.escapeHtml(label)}"><div class="comparison-grid">${cards.map((content, index) => `<article class="comparison-side comparison-side-${index + 1}"><span class="comparison-index">${String(index + 1).padStart(2, '0')}</span>${content}</article>`).join('')}</div></section>`; } },
  pipeline: { name: 'pipeline', schema: {}, render: (node, context) => { validateAttrs(node, blocks.pipeline); const label = context.translate('pipeline', 'From Markdown to site'); return `<section class="block pipeline" aria-label="${context.escapeHtml(label)}"><div class="pipeline-track">${pipelineSteps(node.children, context)}</div></section>`; } },
  'post-list': { name: 'post-list', schema: { limit: 'number' }, defaults: { limit: '6' }, dependencies: (_node, context) => [`collection:posts:${context.doc.locale}`], render: (node, context) => { validateAttrs(node, blocks['post-list']); const limit = numberAttr(node, 'limit', 1, 50, 6); const posts = context.collection('posts').slice(0, limit); const heading = context.translate('latestPosts', 'Latest articles'); return `<section class="block post-list"><div class="post-list-heading"><h2>${context.escapeHtml(heading)}</h2><a href="${context.safeUrl(`/${context.doc.locale}/posts/`)}">${context.escapeHtml(context.translate('allPosts', 'View all articles'))}</a></div>${posts.length ? `<div class="post-list-grid">${posts.map((post, index) => `<article class="post-card">${postCover(post, context, index)}<div class="post-card-body"><h3><a href="${context.safeUrl(context.routeFor(post))}">${context.escapeHtml(post.title)}</a></h3>${post.date ? `<time datetime="${context.escapeHtml(post.date)}">${context.escapeHtml(context.formatDate(post.date))}</time>` : ''}${postExcerpt(post, context) ? `<p class="post-excerpt">${postExcerpt(post, context)}</p>` : ''}</div></article>`).join('')}</div>` : `<p class="empty">${context.escapeHtml(context.translate('noPosts', 'No articles yet.'))}</p>`}</section>`; } },
...
function shell(context: ThemeShellContext) {
  const pageLanguages = context.doc.source.startsWith('generated:') ? '' : context.languageLinks;
