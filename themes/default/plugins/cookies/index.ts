...
export function renderCookieConsent(context: ThemeShellContext): { markup: string; triggerMarkup: string } {
  ...
  const gatedMarkup = privacy.gatedScripts.map(script => `<template data-cookie-script data-cookie-purpose="${escape(script.purpose)}" data-cookie-src="${context.safeUrl(script.href)}"></template>`).join('');
...
export const plugin: ThemePluginDefinition = {
  ...
  defaults: {
    ...
    retentionDays: 365,
    categories: [
      { purpose: 'essential', required: true, default: true, retentionDays: 365 },
      { purpose: 'measurement', required: false, default: false, retentionDays: 0 },
      { purpose: 'advertising', required: false, default: false, retentionDays: 0 },
      { purpose: 'fraud-prevention', required: false, default: false, retentionDays: 0 },
      { purpose: 'social-embedding', required: false, default: false, retentionDays: 0 }
    ],
    // Provider names and fields mirror public web integrations. The browser
    // module owns fixed endpoints; each provider's purpose is registered here.
    integrations: [
      ...
      { provider: 'cloudflare-web-analytics', enabled: false, token: '', purpose: 'measurement' },
      { provider: 'baidu-tongji', enabled: false, siteSignature: '', purpose: 'measurement' },
      { provider: 'recaptcha', enabled: false, siteKey: '', purpose: 'fraud-prevention' },
      { provider: 'hcaptcha', enabled: false, siteKey: '', purpose: 'fraud-prevention' },
      { provider: 'turnstile', enabled: false, siteKey: '', purpose: 'fraud-prevention' },
      { provider: 'x-for-websites', enabled: false, purpose: 'social-embedding' }
    ],
    gatedScripts: [],
  ...
  schema: {
    ...
    categories: {
      ...
      items: {
        type: 'object',
        properties: {
          // purpose is the code-owned reason for processing; legacy id input
          // is normalized by the compiler before this schema is validated.
    ...
    // Locale maps may contain only the labels/purposes translated so far.
