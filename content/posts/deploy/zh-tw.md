...
如果代管商需要產生 Worker、Pages Function 或 hosting 設定，在 `config.yml` 使用正式 target 和靜態目錄，讓 `npm run g` 產生對應產物：
...
target 只選擇要產生的產物，不會讓網站設定取得保存 Provider token 的權限。憑證放在代管商的 secret store 或環境變數中。靜態 Git 整合本身不能把 `backend/handler.ts` 打包成同一個 Worker；需要執行時 API 時，應使用代管商支援的 Worker/Functions 工作流程。

## 4. 檢查產生結果

執行 `npm run g` 後，確認 `dist/public` 裡有首頁、多語言路由、資源、Feed、sitemap、`robots.txt` 和產生的探索檔案。目標需要時，Pageskill 還會在公開快照旁邊產生私有執行時資料；不要把它複製到公開目錄。
...
公開目錄是 `dist/public`，不是專案根目錄，也不是私有的 `dist` 根目錄。不要把 access token、SSH key 或 backend secret 寫入 YAML、Markdown 或公開產生檔案。如果代管商不能執行 `npm run g`，就在 CI 中建置，再透過代管商文件規定的方式上傳 `dist/public` 產物。
