...
真實 schema 比這個閱讀示例更具體，會在使用實例設定前驗證類別和腳本欄位。
...
把審核過的政策放在 `content/pages/privacy/<locale>.md`。不要在 YAML 或 Markdown 中放 HTML、JavaScript、CSS、提供者程式碼或隱藏的腳本 URL。設定是資料，可執行行為由外掛模組負責。
...
標籤和中繼資料作為文字轉義；政策連結和受同意控制的腳本 URL 經過 `safeUrl`，不安全協定會被拒絕。瀏覽器腳本使用 DOM API 建立元素，並且只在同意後接受 `http` 或 `https` 來源。外掛介面沒有 `innerHTML`、`eval`、任意屬性或設定注入的標記。
...
用全新的瀏覽器工作階段確認選擇前不會載入可選腳本。接受一個可選類別，確認經過審核的腳本載入；重新開啟 Cookie 設定，儲存「僅必要項目」，確認後續載入停止。同時檢查政策連結、鍵盤焦點、語言連結，以及每種啟用語言中的 nav/footer 插入效果。

## 成功結果

Cookie 選擇器成為一個可重用的主題外掛，擁有本地化介面、明確的類別中繼資料、安全的同意後載入和經過審核的政策連結。post 仍然是 Markdown，主題也可以增加少量殼層連結，而不會獲得 HTML 或腳本注入入口。

## 常見問題

不要預設開啟分析，不要把未知第三方 URL 當成可信，也不要以為撤回同意可以撤銷之前的腳本工作。不要給外掛增加 `language` 設定，也不要在每個 post 複製選擇器。如果翻譯不完整，讓設定的 fallback 填補缺少的介面 key，再有計畫地完成內容翻譯。
