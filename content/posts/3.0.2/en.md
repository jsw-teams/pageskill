...
7. Configure provider instances as the canonical array in `themes/<name>/theme.yml`, keep optional categories disabled by default, and use the category mapping documented in the Cookie tutorial. The compiler accepts the old object-shaped keys during migration, mapping `conversionId` to Google Ads `tagId` and `siteId` to Baidu `siteSignature`; new files should use the real provider fields. Extra schema fields are accepted for extension, but a provider only runs after its code module is registered and consent is granted.
