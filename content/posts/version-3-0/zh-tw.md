...
- 穩定頁面放在 `content/pages/`，不需要日期；教學、部落格、產品記錄和版本文章放在 `content/posts/`，必須有 `date`。
- 增加本地搜尋、文章目錄和可重用外掛開發的簡短三語文章。

## 從舊內容遷移

1. 保留備份，複製目前儲存庫，執行 `npm install` 和 `npm run g`，把教學入口改到[十分鐘開始你的網站](/zh-tw/posts/start/)。
2. 穩定頁面放在 `content/pages/<id>/<locale>.md`，不補日期；教學和部落格文章移到 `content/posts/<id>/`，提供對應語言檔案和必填 `date`。
