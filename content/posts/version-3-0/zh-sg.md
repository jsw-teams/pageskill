...
- 稳定页面放在 `content/pages/`，不需要日期；教程、博客、产品记录和版本文章放在 `content/posts/`，必须有 `date`。
- 增加本地搜索、文章目录和可复用插件开发的简短三语文章。

## 从旧内容迁移

1. 保留备份，克隆当前仓库，运行 `npm install` 和 `npm run g`，把教程入口改到[十分钟开始你的站点](/zh-sg/posts/start/)。
2. 稳定页面放在 `content/pages/<id>/<locale>.md`，不补日期；教程和博客文章移到 `content/posts/<id>/`，提供对应语言文件和必填 `date`。
