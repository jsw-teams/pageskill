---
kind: post
title: 開發一個可重用元件
description: 增加一個自帶資源的主題元件，只註冊一次，並在每個頁面重用。
date: 2026-09-08
category: tutorial
---

# 開發一個可重用元件

主題元件負責共用的瀏覽器行為、樣式和 messages。只註冊一次；post 繼續寫 Markdown，不複製 HTML。

## 1. 建立一個模組目錄

```text
themes/default/components/reading-tip/
  index.ts
  script.js
  style.css
  messages.yml
```

目錄自己保存資源。`themes/default/index.ts` 是組裝入口，由 `theme.config` 選擇的檔案（通常是 `site/theme.yml`）保存這個網站的元件覆寫項。

## 2. 先設定已登記的元件

改程式碼前先查看元件 schema，再在 `site/theme.yml` 使用安全的外觀選項。基礎元件的開關、限制、shell 插入點和可選本地化文案屬於實例資料。第三方 Integration 不同：只在根級 `integrations` 設定真正使用的 Provider 識別，purpose 和同意中繼資料由適配器自己擁有：

```yaml
components:
  search:
    enabled: true
    maxResults: 8
    # 只翻譯部分語言時，缺少的 key 會從 messages.yml 回退。
    copy:
      zh-tw:
        placeholder: 搜尋本主題
  toc:
    enabled: true
    maxDepth: 4
```

語言啟用不是元件選項；`activeLocales` 和 `i18n.fallbackLocale` 放在 `config.yml` 或其擴充檔案。新增語言只完成 50% 翻譯也可以產生：缺少的介面 key 使用回退語言，已存在的 Markdown 檔案則完全按原文顯示。

## 3. 匯出元件定義

在 `index.ts` 使用公開的 `ComponentDefinition`：

```ts
import type { ComponentDefinition } from '../../../../src/theme-api.ts';

export const component: ComponentDefinition = {
  id: 'reading-tip',
  capabilities: ['render', 'client'],
  implementation: 'components/reading-tip/index.ts',
  resources: {
    // 讓模組擁有自己的資源，編譯器才能統一追蹤和指紋化。
    styles: ['components/reading-tip/style.css']
  },
  client: { module: 'components/reading-tip/script.js', selector: 'main' },
  i18n: 'components/reading-tip/messages.yml',
  defaults: { enabled: true },
  schema: { enabled: { type: 'boolean' } }
};
```

在主題的 `components` 陣列註冊一次：

```ts
import { component as shell } from './shell/chrome.ts';
import { component as privacyConsent } from './consent/index.ts';
import { component as readingTip } from './reading-tip/index.ts';

export const components = [shell, search, toc, postMeta, privacyConsent, language, readingTip];
```

## 4. 寫入最小可運作資源

`script.js`：

```js
// Core 尋找 root，並注入受控生命週期 runtime。
export function mount(root, runtime) {
  const marker = document.createElement('small');
  marker.className = 'reading-tip';
  marker.textContent = 'Reading tip enabled';
  root.prepend(marker);
}
```

每個瀏覽器模組都使用統一 Runtime 契約。DOM 與產生資源不需要 API 宣告。需要資料庫、Cache、模型、由私密憑證支援的操作或寫入時，增加 `api: 'service-id'` 並呼叫 `runtime.apiJson('relative/path')`。根層 `config.apis.<service-id>` 決定絕對 URL，讓每個 Component 只能存取命名服務。設定 Token 是公開瀏覽器資料；私密憑證必須透過獨立部署的代理。

`style.css`：

```css
/* 樣式和元件資源放在一起，刪除時不會留下孤立依賴。 */
.reading-tip { margin-inline-start: .5rem; }
```

`messages.yml`：

```yaml
messages:
  en:
    readingTip:
      # 介面文案放這裡，執行行為仍由 index.ts/script.js 負責。
      label: Reading tip
```

## 5. 在 site/theme.yml 保留實例開關

```yaml
# site/theme.yml
components:
  readingTip:
    # 不改 renderer 就能開關已登記的能力。
    enabled: true
```

在複製下來的網站執行檢查：

```powershell
npm run compile-theme
page g
page s
```

## 成功結果

產生的頁面會載入元件腳本和樣式，主要內容區域出現標記。把 `site/theme.yml` 中的 `components.readingTip.enabled` 改為 `false` 後重新產生即可移除；新增文章不需要再寫 HTML。

產生時會把模組的資源和 messages 收集到公開主題資源中，未變動的公開資源繼續使用原內容 hash 路徑和快取身分。

## 6. 以 Cookie 選擇器作為參考

本主題的 [Cookie 選擇器教學](/zh-tw/posts/cookies/)是一個完整參考實作。它在同樣的模組結構上加入 schema、本地化訊息、同意後瀏覽器行為和安全渲染；當元件不只需要一個資源時，可以依照這個結構擴充。

普通導覽和頁尾連結是 `config.yml` 或其擴充檔案中的網站資料；主題插槽仍透過 `site/theme.yml` 的 `components.shell` 設定。不要讓元件腳本向 `.site-header` 或 `.site-footer` 任意追加連結。shell Component 只接受結構化標籤和安全 URL；像本例這樣的頁面級行為仍可掛載到 `main` 內。

如果元件要向瀏覽器 Agent 暴露 WebMCP 工具，先按[設定條件式 Agent 能力](/zh-tw/posts/agent-discovery/)在元件腳本中登記真實的 `document.modelContext` 工具，再另外開啟 `config.yml` 的探索宣告；這個開關不會替元件載入腳本。

## 7. 乾淨地移除元件

能力不再需要時，刪除主題組裝入口中的 import、元件定義和資源登記、`site/theme.yml` 覆寫項，以及 Markdown directive 或 shell 參照。重新產生並檢查 catalog；不要只刪產生的資源，也不要為舊消費者保留第二套實作。

## 常見問題

資源路徑都相對主題根目錄。不要修改 `dist/`，不要把程式碼放進 `config.yml`，也不要讓每篇文章直接匯入元件。

## 下一步

閱讀[把網站放到網上](/zh-tw/posts/deploy/)，了解主機如何發布生成的 `dist/public` 快照。
