...
- 增加 `content/updates/<version>/<locale>.md` 的 `updates` 文章集合，拥有独立的本地化索引/详情路由、归档、Feed、搜索结果、语言链接、导航和首页栏目。移动 3.0.0 与 3.0.1 说明时保留日期、作者和封面；教程继续放在 `content/posts/`。
- 复用现有 collection 驱动的归档、Feed 和文章列表机制，没有新增第二套页面 Pattern。更新日志的前后文章关系只在 updates 集合内，生成的归档页现在有可见标题、说明和本地化链接。
