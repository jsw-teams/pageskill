...
Pageskill 4.0.0 是一次 breaking architecture release。当前产品模型统一为：`Content + Configuration + Theme + Components + 静态生成 + 无障碍 + Agent readiness + 可选 Runtime Adapter`。

### 变更

- `ComponentDefinition` 成为唯一公开 Theme 扩展概念。Built-in 与 External 只是来源标签；`render`、`client`、`server`、`storage`、`cache`、`ai`、`integration` 都是同一个 Component 的能力，不再保留 Plugin、Pattern、Layout、Module 或独立 Block 注册模型。
- 正式落实“Component 负责行为和表现；Content 负责内容”。Component 接收 children、slots、structured props、Runtime Data、Core content query 和 URL resolver；默认 Theme 不再拥有 Demo 站点的文章 ID、页面正文、分类、locale 路径或品牌数据。
- Component 的 `messages.yml` 只保存短 UI 文案、状态和 ARIA 文案。站点正文、Feature 描述、法律文本和示例留在 Markdown 或站点配置；Hero、Feature Grid、Learning Path、CTA、Post List、文档与归档改为内容驱动。
...
- 静态站点省略 `runtime`；只有实际部署参考 Pages/Functions/D1/Workers AI 运行时时才选择 `runtime.adapter: cloudflare-pages`。
