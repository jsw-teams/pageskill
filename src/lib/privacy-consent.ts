...
export function renderPrivacyConsent(privacy: ComponentPrivacyContext, tools: PrivacyRenderTools): { markup: string; triggerMarkup: string } {
  if (!privacy.providerEnabled) return { markup: '', triggerMarkup: '' };
  ...
  if (!privacy.enabled) {
    return {
      markup: `<section class="privacy-consent" data-cookie-consent data-cookie-ui="false" data-cookie-version="1" data-cookie-decision-retention-days="${privacy.decisionRetentionDays}" data-cookie-integrations="${integrationMarkup}"${placeholderAttributes}></section>`,
      triggerMarkup: ''
    };
  }
